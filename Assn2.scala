...
object Assn2 {
  ...
  object Value {
    ...
    println(val x = multiply(Num(3),Num(2)))

    val y = multiply(Num(3),Bool(true))

    def eq(v1: Value, v2: Value): Value = (v1,v2) match{
      case _ if(NumV(v1) == NumV(v2)) => true
      case _ if(StringV(v1) == StringV(v2)) => true
      case _ if(BoolV(v1) == BoolV(v2)) => true
